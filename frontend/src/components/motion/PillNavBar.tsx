...
type Props = {
  ...
  /** When true, hide built-in hamburger (parent handles mobile menu). */
  desktopOnly?: boolean;
};

/** React Bits PillNav — animated logo + pill links (desktop storefront chrome). */
export function PillNavBar({
  ...
  return (
    <div
      className={`pill-nav-bar relative z-[40] w-full ${desktopOnly ? "hidden md:block" : ""} ${className}`.trim()}
    >
      <nav
      ...
      {!desktopOnly ? (
        <div
          ref={mobileMenuRef}
          className="md:hidden absolute top-[3.25em] left-0 right-0 rounded-[27px] z-[998] origin-top"
          style={{
            ...cssVars,
